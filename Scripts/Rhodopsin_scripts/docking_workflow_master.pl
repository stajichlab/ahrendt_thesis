...
my $ga_run = 50;   # number of genetic algorithm runs
my $ga_gen = 27000; # maximum number of generations
my $user_ligand; # user-specifide single ligand to use
my $cov; # flag for covalent docking
my $ignore_lig; # pdbqt already exists (for whatever reason), so don't prep it from the .mol2 file
...
GetOptions ('l|ligand=s'   => \$ligand, 
            ...
            'bo|bound'     => \$bound,
            'covalent'     => \$cov,
            'ligand=s'     => \$user_ligand,
            'ignoreLig'    => \$ignore_lig);

my $usage = "Usage: docking_workflow.pl -r receptor -a crit_atom [--ligand lig_dir]\nDo not use extensions on the ligand or receptor\n";
die $usage if($help);
#die "No critical atom provided; this is needed for centering\n$usage" if (!$crit_atom);
#$lig_atom = join("/",$user_ligand,$lig_atom) if($user_ligand);
...
my @ligands; # list of ligands to process

if(!$user_ligand)
{
  opendir(DIR,".");
  #@ligands = grep { /A1_\d{3}/ } readdir(DIR);
  @ligands = grep { /LIG_\d{3}/ } readdir(DIR);
  closedir(DIR);
  #print "@ligands";
}
else
{
  push @ligands,$user_ligand; # use a single, user-specified ligand directory
}
